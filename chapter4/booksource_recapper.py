...
def http_assembler(pcap_fl):
    ...
    for session in sessions:
        ...
        for packet in sessions[session]:
            try:
                if packet[TCP].dport == 80 or packet[TCP].sport == 80 or packet[TCP].dport == "http" or packet[TCP].sport == "http":
        ...
        if image is not None and image_type is not None:
            # store the image
            file_name = "pic_carver_%d.%s" % (
                 carved_images, image_type)

# set the timeout value properly for different connection on line 35. 
...
def receive_from(connection, host_type):
    buffer = b''

    connection.settimeout(3 if host_type == 'remote' else 13)
    try:
        while True:
...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
    ...
    if receive_first:
        remote_buffer = receive_from(remote_socket, 'remote')
    ...
    while True:
        local_buffer = receive_from(client_socket, 'client')
        if len(local_buffer):
